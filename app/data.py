...
def database_init(xml_file):
    """Fonction permettant d'initialiser une base de données avec les informations issues de xml_file.
        ...
        (2) Boucle sur chaque élément dive de l'arbre XML :
    ...
    for item in xml_file.xpath("//group/div"):
        ...
        item_date = xml_file.xpath("//div[@n=" + str(number) + "]//date/text()")
        list_date.append(item_date[0])
        item_place = xml_file.xpath("//div[@n=" + str(number) + "]//docDate/placeName/text()")
        list_place.append(item_place[0])
        item_analyse = xml_file.xpath("//div[@n=" + str(number) + "]//argument[1]/p/text()")
